...
async function load() {
  if(section==='team') {
    ...
    if(album) {
      ...
      frame(`<a href="?section=gallery">← 앨범 목록</a><div class="heading"><div><h2>${esc(album.title)}</h2><p>${album.published?'공개 앨범':'숨긴 앨범'}</p></div><button class="primary" data-action="add-photos" data-id="${album.id}">사진 추가</button></div>`+
        (photos.length?`<div class="grid">${photos.map(p=>`<article class="card"><img class="album-cover" loading="lazy" src="${esc(photoUrl(p.path))}" alt="${esc(p.caption)}"><div class="card-body"><p>${esc(p.caption)}</p><p class="small muted">순서 ${p.sort_order} · ${p.active?'공개':'숨김'}</p>${editButton('photo',p.id,'설명·순서·공개 수정')}</div></article>`).join('')}</div>`:empty('사진을 추가해 주세요.')));
    } else frame(toolbar('활동 앨범','album')+table(['앨범','활동일','분류','공개','관리'],albums.map(a=>`<tr><td>${esc(a.title)}</td><td>${esc(a.event_date)}</td><td>${esc(a.category)}</td><td>${a.published?'공개':'숨김'}</td><td><div class="actions">${editButton('album',a.id)}<a href="?section=gallery&album=${a.id}">사진 관리</a></div></td></tr>`)));
...
view.addEventListener('click',event=>{
  ...
  if(action==='photo') {
    ...
    modal('사진 설명·표시 순서',field('caption','사진 설명',p.caption,'required maxlength="180"')+field('sort_order','표시 순서',p.sort_order,'type="number" required')+check('active','앨범에 표시',p.active),async d=>{await saveRecord('album_photos',{caption:d.get('caption').trim(),sort_order:Number(d.get('sort_order')),active:d.has('active')},p);await saved();});return;
  }
